...
from Dam.helpers.tools import get_arg, get_text, resize_media
from Dam.modules.Ubot.help import add_command_help
from Dam.utils.tools import add_text_img, bash
...
async def kang(client: Client, message: Message):
    ...
    if replied and replied.media:
        if replied.photo:
            ...
            return
        media_ = await client.download_media(replied, file_name="Dam/resources/")
    else:
...
async def cb_sticker(client: Client, message: Message):
    ...
    if not query:
        return await message.reply("**Masukan Nama Sticker Pack!**")
    xx = await message.reply("`Searching sticker packs...`")
...
async def tinying(client: Client, message: Message):
    ...
    im1 = Image.open("Dam/resources/blank.png")
    if ik.endswith(".tgs"):
...
async def memify(client: Client, message: Message):
    ...
    if len(text) < 1:
        return await msg.edit(f"Harap Ketik `.mmf text`")
...
add_command_help(
    ...
    [
        [
            f".kang `atau` .tikel",
            f"Balas .kang Ke Sticker Atau Gambar Untuk Menambahkan Ke Sticker Pack.",
        ],
        [
