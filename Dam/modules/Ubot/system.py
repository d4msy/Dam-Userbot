...
from Dam import BOTLOG_CHATID, LOGGER
from Dam.helpers.basic import edit_or_reply
from Dam.helpers.misc import HAPP
...
async def restart_bot(_, message: Message):
    try:
        ...
        return
    await msg.edit_text("☑️ **Bot was restarted!**\n\n")
    if HAPP is not None:
        HAPP.restart()
    else:
...
async def shutdown_bot(client: Client, message: Message):
    if BOTLOG_CHATID:
        await client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Dam-Userbot** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await message.reply("🔌 **Dam-Userbot Berhasil di matikan!**")
    if HAPP is not None:
